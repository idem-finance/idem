...
class AccountControllerTest {
    ...
    private fun accountFor(id: UUID = accountId) =
        Account.create(
            id = AccountId(id),
            tenantId = tenantId,
            name = "USDC Wallet",
    ...
    private fun statementFor(accountId: UUID) =
        AccountStatement(
            accountId = AccountId(accountId),
            currency = FiatCurrency.BRL,
    ...
    fun `asOf parameter is forwarded to query`() {
        ...
        mockMvc
            .get("/api/v1/accounts/$accountId/balance?asOf=$asOf") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isOk() }
            }
    ...
    fun `listEntries with limit 0 returns 400 INVALID_LIMIT`() {
        mockMvc
            .get("/api/v1/accounts/$accountId/entries?limit=0") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isBadRequest() }
    ...
    fun `listEntries with limit 201 returns 400 INVALID_LIMIT`() {
        mockMvc
            .get("/api/v1/accounts/$accountId/entries?limit=201") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isBadRequest() }
                jsonPath("$.code") { value("INVALID_LIMIT") }
    ...
    fun `listEntries with from after to returns 400 INVALID_RANGE`() {
        mockMvc
            .get("/api/v1/accounts/$accountId/entries?from=2026-05-28T00:00:00Z&to=2026-05-01T00:00:00Z") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isBadRequest() }
    ...
    fun `listEntries with invalid cursor returns 400 INVALID_CURSOR`() {
        ...
        mockMvc
            .get("/api/v1/accounts/$accountId/entries?cursor=garbage") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isBadRequest() }
    ...
    fun `listEntries forwards from, to, limit and cursor into the query`() {
        ...
        mockMvc
            .get("/api/v1/accounts/$accountId/entries?from=2026-05-01T00:00:00Z&to=2026-05-28T00:00:00Z&limit=10&cursor=abc") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isOk() }
            }
    ...
    fun `statement happy path returns 200 with opening, closing and movements`() {
        ...
        mockMvc
            .get("/api/v1/accounts/$accountId/statement?from=2026-05-01T00:00:00Z&to=2026-05-28T00:00:00Z") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isOk() }
                jsonPath("$.accountId") { value(accountId.toString()) }
    ...
    fun `statement missing from returns 400 MISSING_PARAMETER`() {
        mockMvc
            .get("/api/v1/accounts/$accountId/statement?to=2026-05-28T00:00:00Z") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isBadRequest() }
    ...
    fun `statement missing to returns 400 MISSING_PARAMETER`() {
        mockMvc
            .get("/api/v1/accounts/$accountId/statement?from=2026-05-01T00:00:00Z") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isBadRequest() }
                jsonPath("$.code") { value("MISSING_PARAMETER") }
    ...
    fun `statement with from after to returns 400 INVALID_RANGE`() {
        ...
        mockMvc
            .get("/api/v1/accounts/$accountId/statement?from=$from&to=$to") {
                with(SecurityMockMvcRequestPostProcessors.authentication(mockAuth("ACCOUNTS_READ")))
            }.andExpect {
                status { isBadRequest() }
                jsonPath("$.code") { value("INVALID_RANGE") }
