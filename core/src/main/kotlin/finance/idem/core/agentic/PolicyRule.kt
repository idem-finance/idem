...
sealed class PolicyRule {
     ...
     */
    data class RequireHumanApprovalAbove(
        val threshold: MonetaryAmount,
    ) : PolicyRule()
     ...
     */
    data class AllowedTokens(
        val tokens: Set<StablecoinToken>,
    ) : PolicyRule()
     ...
     */
    data class AllowedChains(
        val chains: Set<ChainId>,
    ) : PolicyRule()
    ...
    fun params(): Map<String, Any> =
        when (this) {
            is MaxDebitPerSession -> {
                mapOf("amount" to limit.value.toPlainString())
            }
            ...
            is RequireHumanApprovalAbove -> {
                mapOf("amount" to threshold.value.toPlainString())
            }

            is ForbiddenAccountPair -> {
                mapOf(
            ...
            is AllowedTokens -> {
                mapOf("tokens" to tokens.map { it.name })
            }

            is AllowedChains -> {
                mapOf("chains" to chains.map { it.name })
            }
        }
}
