...
class ApiKeyRepositoryAdapter(
    ...
    override fun findByPrefix(prefix: String): ApiKey? =
        jpaRepository.findByPrefix(prefix)?.toDomain()
...
private fun ApiKeyDataModel.toDomain(): ApiKey = ApiKey(
    ...
    keyHash = keyHash,
    prefix = prefix,
    scopes = scopes.split(",").filter { it.isNotEmpty() }
        .mapTo(mutableSetOf()) { ApiScope.valueOf(it) },
    createdAt = createdAt,
    revokedAt = revokedAt,
)

private fun ApiKey.toEntity(): ApiKeyDataModel = ApiKeyDataModel(
    id = id.value,
