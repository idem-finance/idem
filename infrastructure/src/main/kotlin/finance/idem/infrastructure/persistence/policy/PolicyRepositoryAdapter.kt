...
class PolicyRepositoryAdapter(
    ...
    override fun findEffective(tenantId: TenantId, agentKeyPrefix: String?): List<PolicyRule> {
        entityManager.setRlsTenantId(tenantId)
        val agentFilter = if (agentKeyPrefix != null) {
            " OR agent_key_prefix = '${agentKeyPrefix.replace("'", "''")}'"
        } else ""
        val sql = "SELECT * FROM policy_rules WHERE tenant_id = '${tenantId.value}'" +
            " AND enabled = true AND (agent_key_prefix IS NULL$agentFilter)"
        @Suppress("UNCHECKED_CAST")
        val rows = entityManager.createNativeQuery(sql, PolicyRuleDataModel::class.java)
    ...
    override fun delete(tenantId: TenantId, ruleId: PolicyRuleId): Boolean {
        entityManager.setRlsTenantId(tenantId)
        val existing = jpaRepository.findByIdAndTenantId(ruleId.value, tenantId.value)
            ?: return false
    ...
    private fun PolicyRuleDataModel.toRule(): PolicyRule {
        ...
        return when (ruleType) {
            ...
            else -> throw IllegalStateException("Unknown rule_type: $ruleType")
        }
    }
}
