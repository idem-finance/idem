...
class AuditExportRepositoryAdapter(
    ...
    override fun findForExport(query: ExportAuditLogQuery): List<AuditExportRecord> {
        ...
        val humanRecords = when (query.type) {
            AuditEntryType.AGENT -> emptyList()
            else -> auditLogJpaRepository.findForExport(
                query.tenantId.value, query.from, query.to,
            ).map { it.toExportRecord() }
        }
        val agentRecords = when (query.type) {
            AuditEntryType.HUMAN -> emptyList()
            else -> agentAuditEventJpaRepository.findForExport(
                query.tenantId.value, query.from, query.to,
            ).map { it.toExportRecord() }
        }
    ...
    private fun AgentAuditEventDataModel.toExportRecord() = AuditExportRecord(
        timestamp = occurredAt,
        actor = agentId,
        action = when (AgentAuditStatus.valueOf(status)) {
