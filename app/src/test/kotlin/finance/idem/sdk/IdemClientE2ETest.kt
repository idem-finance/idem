...
class IdemClientE2ETest {
    ...
    private fun insertAccount(accountId: UUID, tenantId: TenantId, name: String, type: String) {
    ...
    private fun countTransactionsById(tenantId: TenantId, transactionId: UUID): Int {
        dataSource.connection.use { conn ->
            conn.autoCommit = false
            conn.createStatement().execute("SET LOCAL app.tenant_id = '${tenantId.value}'")
    ...
    private fun countTransactionsByIdempotencyKey(tenantId: TenantId, idempotencyKey: String): Int {
        dataSource.connection.use { conn ->
            conn.autoCommit = false
            conn.createStatement().execute("SET LOCAL app.tenant_id = '${tenantId.value}'")
            val count = conn.prepareStatement(
    ...
    @Test
    fun `postTransaction commits a balanced PIX transaction and returns a transactionId`() = runBlocking {
    ...
    @Test
    fun `postTransaction with the same Idempotency-Key replays the same transactionId and writes one row`() = runBlocking {
        val tenantId = TenantId.generate()
        val (rawKey, _) = apiKeyService.generate(tenantId, setOf(ApiScope.TRANSACTIONS_WRITE, ApiScope.ACCOUNTS_READ))
        val cashAccount = UUID.randomUUID()
        val payableAccount = UUID.randomUUID()
    ...
    @Test
    fun `getBalance reflects a posted credit on the liability account`() = runBlocking {
        val tenantId = TenantId.generate()
        val (rawKey, _) = apiKeyService.generate(tenantId, setOf(ApiScope.TRANSACTIONS_WRITE, ApiScope.ACCOUNTS_READ))
        val cashAccount = UUID.randomUUID()
        val payableAccount = UUID.randomUUID()
    ...
    @Test
    fun `listEntries returns journal lines only for the requested account`() = runBlocking {
        val tenantId = TenantId.generate()
        val (rawKey, _) = apiKeyService.generate(tenantId, setOf(ApiScope.TRANSACTIONS_WRITE, ApiScope.ACCOUNTS_READ))
        val cashAccount = UUID.randomUUID()
        val payableAccount = UUID.randomUUID()
    ...
    @Test
    fun `getStatement opening and closing balances match the posted movement`() = runBlocking {
        val tenantId = TenantId.generate()
        val (rawKey, _) = apiKeyService.generate(tenantId, setOf(ApiScope.TRANSACTIONS_WRITE, ApiScope.ACCOUNTS_READ))
        val cashAccount = UUID.randomUUID()
        val payableAccount = UUID.randomUUID()
    ...
    @Test
    fun `request with an unknown API key throws ApiException with status 401`() = runBlocking {
    ...
    @Test
    fun `postTransaction without TRANSACTIONS_WRITE scope throws ApiException with status 403`() = runBlocking {
        val tenantId = TenantId.generate()
