...
class IdemMcpServer(
    ...
    @Tool(
        description =
            "Post a double-entry ledger transaction as an AI agent. Requires AGENTS_EXECUTE scope. " +
                "Policy rules are evaluated before commit; a PolicyViolationException is thrown if any rule is violated.",
    )
    fun postTransaction(
        @ToolParam(
            description =
                "Journal lines: each line specifies accountId, entryType (DEBIT/CREDIT), " +
                    "monetaryEntryType (FIAT/ON_CHAIN), amount, and type-specific fields",
        ) entries: List<McpJournalLineInput>,
    ...
    @Tool(
        description =
            "List journal entries for an account, newest first. Supports time-range filtering " +
                "and cursor-based pagination. Requires AGENTS_EXECUTE scope.",
    )
    fun listEntries(
        @ToolParam(description = "Account UUID") accountId: String,
        ...
        return getEntriesUseCase.execute(query).fold(
            onSuccess = { page ->
                EntryListResult(
                    ...
                    entries =
                        page.entries.map { line ->
                            EntryItem(
                                id = line.id.toString(),
                                transactionId = line.transactionId.value.toString(),
                                entryType = line.entryType.name,
                                amount =
                                    line.monetaryEntry.amount.value
                                        .toPlainString(),
                                currency =
                                    when (val me = line.monetaryEntry) {
    ...
    @Tool(
        description =
            "Roll back a committed or executing workflow using compensating transactions (saga pattern). " +
                "Each executed step is reversed in reverse order. Requires AGENTS_ROLLBACK scope.",
    )
    ...
    @Tool(
        description =
            "Run a reconciliation sweep over on-chain settlements within a time window. Matches UNMATCHED " +
                "chain entries against PENDING journal lines by amount. Requires AGENTS_EXECUTE scope.",
    )
    fun reconcileBatch(
    ...
    @Tool(
        description =
            "Retrieve HMAC-signed audit events for agent actions. Filterable by session, time range, and count. " +
                "Each event includes hmacSignature for integrity verification. Requires AGENTS_AUDIT_READ scope.",
    )
    fun getAgentAuditLog(
        ...
        val items =
            getAgentAuditLogUseCase.execute(query).map {
                AuditEventItem(
                    id = it.id.toString(),
    ...
    private fun handleFailure(error: Throwable): Nothing =
        when (error) {
            is PolicyViolationException -> {
                throw error
            }
...
data class McpJournalLineInput(
    ...
    fun toJournalLineRequest(): JournalLineRequest {
        val monetaryEntry =
            when (monetaryEntryType.uppercase()) {
                "FIAT" -> {
                    FiatEntry(
                ...
                "ON_CHAIN" -> {
                    ...
                    throw IllegalArgumentException("Unknown monetaryEntryType: $monetaryEntryType")
                }
            }
