...
**Package:** `finance.idem.core.events` (enums) / `finance.idem.application.events` (value object)

An append-only, event-sourcing-style record of every domain mutation — NOT an event bus.
Written synchronously in the same `@Transactional` as the mutation it records, alongside
`audit_log` and `webhook_outbox`, per the "Event publishing — MVP rule (NO event bus)" in
`CLAUDE.md`. No Kafka, no `ApplicationEventPublisher`, no async dispatch.

### DomainEventType

```
TRANSACTION_COMMITTED, TRANSACTION_SETTLED,
RECONCILIATION_UNMATCHED, RECONCILIATION_EXCEPTION,
WORKFLOW_COMMITTED, WORKFLOW_ROLLED_BACK,
COMPLIANCE_TRAVEL_RULE_REQUIRED, AGENT_ACTION_FLAGGED
```

Backed by a DB `CHECK` constraint on `domain_events.event_type` (kept in sync with this enum
by `FlywayMigrationTest`). Unifies three previously-divergent event-naming schemes:
`webhook_outbox.event_type` (raw dot-strings), `agent_audit_events.status` (generic
PENDING/COMPLETED/FAILED), and an MCP-read-time-only derived label. `AGENT_ACTION_FLAGGED` is
new — a PolicyGuard denial previously had no representation beyond a generic `FAILED` status
and free-text outcome.

### DomainEvent

```
DomainEvent(
    id: UUID, tenantId: TenantId, eventType: DomainEventType,
    referenceId: UUID, referenceType: DomainEventReferenceType,
    correlationId: String, occurredAt: Instant,
)
```

`referenceId`/`referenceType` (`TRANSACTION` or `WORKFLOW`) point at what the event is about —
deliberately not reusing a single typed column to carry two different ID kinds, unlike
`WebhookOutboxEntry.transactionId`, which also stores a `workflowPlanId` for workflow events.

`correlationId` carries the request-scoped trace ID already generated by `TraceIdFilter`
(`X-Idem-Trace-Id` header / SLF4J MDC), resolved at each write site via
`TraceContext.currentOrNew()`. Because MDC is thread-local, every event written during one
HTTP request — including a multi-step agent workflow's per-step transactions and its final
`WORKFLOW_COMMITTED` event — naturally shares one `correlationId` with no explicit threading.
Writes with no live HTTP request (a `@Scheduled` reconciliation sweep) fall back to a fresh
UUID per event. There is deliberately no `causationId`: nothing in this codebase reacts
asynchronously to a persisted `DomainEvent` to produce another one, so there is no genuine
event-caused-event edge to model yet.

For a PolicyGuard denial specifically, the `AGENT_ACTION_FLAGGED` event is written by
`AgentAuditRecorder.recordDurable(event, domainEvent)` in the same `REQUIRES_NEW` transaction
as the denial's `AgentAuditEvent`, since the caller's business transaction rolls back on
`PolicyViolationException` and would otherwise discard it.
...
Lives in `application/port` (not `core`) — mirrors `AuditRepository`'s shape exactly.
